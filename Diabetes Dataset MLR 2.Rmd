...
reduced <- lm(glyhb~stab.glu+age+time.ppn, data=df)
```
...
```
#????????/
The F statistic from this test is 1.8599, with a p-value of 0.04928. We fail to reject the null hypothesis. There is little evidence for supporting the full model. We go with the reduced model over the full model.
